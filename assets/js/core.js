function playTrack(id){
	$.ajax({
		url: SITE_URL+"/track/play",
		data: {id: id, "csrf_token_rs": CSRF_HASH},
		dataType: 'html',
		type: 'GET',
		success: function(html){
			$('#player_modal').html(html).modal("show");
		}
	});
}
...
//**********************************MAIN MENU********************************
	...
	function layout_condensed_toggle(){
		if($('#main-menu').attr('data-inner-menu')=='1'){
			...
			if(readCookies("main_menu") != "mini"){
				$('#main-menu').removeClass('mini');
				$('.page-content').removeClass('condensed');
				$('.scrollup').removeClass('to-edge');
				$('.header-seperation').show();
				$('.footer-widget').show();
				$('.page-sidebar li').tooltip('disable');
			}
			else
			{
				...
				$('.footer-widget').hide();
				$('.page-sidebar li').tooltip('enable');
			}
		}
	}
	...
	$('#layout-condensed-toggle').click(function(){
		if($('#main-menu').attr('data-inner-menu')=='1'){
			...
			{
				document.cookie = "main_menu=mini";
				$('#main-menu').addClass('mini');
				$('.page-content').addClass('condensed');
				$('.scrollup').addClass('to-edge');
				$('.header-seperation').hide();
    ...
    var progressRadio = function () {
	  $.getJSON(SITE_URL+'/stream/data', function (response) {
	    $('.progress-radio .details').html("&nbsp;");
	    if( response.now == "OFF" ){
	    	$('.progress-radio .progress').removeClass("progress-success").addClass("progress-danger");
	    	$('.progress-radio .progress .bar').css('width', 100+"%");
	    	setTimeout(progressRadio, response.progress.next*1000);
	    	$('.progress-radio .details').append("<small class='pull-left'><i class='icon-stop'></i> "+response.now+"</small>");
	    }else{
	    	$('.progress-radio .details').append("<small class='pull-left'><i class='icon-play'></i> "+response.now+"</small>");
		    if( response.next != null && response.next != "" ){
		    	$('.progress-radio .details').append("<small class='pull-right'><b>NEXT:</b>"+response.next+"</small>");
		    }
		    $('.progress-radio .progress .bar').css('width', response.progress.percent+"%");
		    setTimeout(progressRadio, response.progress.next*1000);
		    $('.progress-radio .progress .bar').stop().animate({'width':100+"%"}, response.progress.next*1000);
	    }
	  });
	};
	progressRadio();
